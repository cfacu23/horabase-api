...
export function MyShiftsPage() {
  const period = useMemo(() => range(14, 60), []); const shifts = useApiData<Shift[]>(`/api/me/calendar?from=${encodeURIComponent(period.from.toISOString())}&to=${encodeURIComponent(period.to.toISOString())}`, [])
...
export function MyAttendancesPage() {
  const period = useMemo(() => range(90), []); const records = useApiData<Attendance[]>(`/api/me/attendances?from=${encodeURIComponent(period.from.toISOString())}&to=${encodeURIComponent(period.to.toISOString())}`, [])
...
export function MyIncidentsPage() {
  const period = useMemo(() => range(180), []); const records = useApiData<Incident[]>(`/api/me/incidents?from=${period.from.toISOString().slice(0, 10)}&to=${period.to.toISOString().slice(0, 10)}`, [])
...
export function MyOvertimePage() {
  const period = useMemo(() => range(180), []); const records = useApiData<Overtime[]>(`/api/me/overtime?from=${period.from.toISOString().slice(0, 10)}&to=${period.to.toISOString().slice(0, 10)}`, [])
